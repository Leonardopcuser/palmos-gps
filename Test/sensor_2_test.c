...
void loop(void)
{
  ...
  if (millis() - timer > time_delay) {
    ...
    char tempCelcius[] = "25.622";
    char humidityData[] = "55.1234";
    char acc_x[] = "0.3206";
    char acc_y[] = "0.-1625";
    char acc_z[] = "10.1551";
  ...
  // if millis() or timer wraps around, we'll just reset it
  if (timer_2 > millis()) timer_2 = millis();
     
  // approximately every 2 seconds or so, print out the current stats
  if (millis() - timer_2 > 5*time_delay) {
