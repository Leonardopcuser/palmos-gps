...
bool Adafruit_FXOS8700::getEvent(sensors_event_t* accelEvent, sensors_event_t* magEvent)
{
  ...
  //Serial.println("getEvent Entered");
  /* Clear the event */
  memset(accelEvent, 0, sizeof(sensors_event_t));
  // Serial.println("memset accelEvent done, next");
  memset(magEvent, 0, sizeof(sensors_event_t));

  //Serial.println("memset entered, clearing raw data placeholder");
  ...
  //Serial.println("Raw data placeholder cleared");
  ...
  //Serial.println("static metadata set");


  /* Read 13 bytes from the sensor */
  Wire.beginTransmission((byte)FXOS8700_ADDRESS);

  //Serial.println("transmission began to byte value address");
  #if ARDUINO >= 100
  ...
  //Serial.println("13 bytes from sensor read");
